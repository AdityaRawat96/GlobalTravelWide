...
$(document).ready(function () {
    ...
    (table = $(data_table)
        ...
        .on("draw", function () {
            exportProductsToOrder(), toggleMultipleActions();
        })),
        exportProductsToOrder();
...
function removeProductFromOrder(e, index) {
    e.preventDefault();
    e.stopPropagation();
    $("#create_order_row_" + index).remove();
}
...
function addProductsToOrderModal(elem, asin = null) {
    ...
    selected_products.forEach((product_input, index) => {
        $("." + elem).append(`
       <tr id="create_order_row_${product_input.value}">
          <!--begin::Product=-->
          <td>
            <button type="button" class="btn btn-sm btn-icon btn-bg-light btn-active-color-primary mt-3" onclick="removeProductFromOrder(event, '${
                product_input.value
            }')">
          ...
          <!--end::Product=-->
         <td class="pe-5">
            <span class="mt-3 fw-bold ms-3 d-block" id="product-stock-${index}">${
            product_input.dataset.stock
        }</span>
          </td>
          <td class="pe-5">
          ...
          <td class="pe-5">
              ...
              <div class="fv-row product_bundling_option_${index} d-none">
                  <!--begin::Input-->
                  <small class="required mb-2" for="outgoing_package_items[${index}]">Enter new ASIN</small>
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if (response.success) {
                ...
                $("#outgoing-product-container-" + index).addClass("d-none");
                $("#outgoing-product-asin-error-" + index).removeClass(
                    "d-none"
                );
            }
        },
