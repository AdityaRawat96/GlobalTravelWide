...
class UserUpdateOrderWizard extends Component
{
    use WithFileUploads;
    ...
    {
        return view('user.order.update-wizard-form');
    }
    ...
    public function compareProducts()
    {
        ...
        if (count($this->products) != count($this->products_list_old)) {
            ...
            foreach ($this->products as $index => $product) {
                ...
                $new_product = $product;
                $old_product = $this->products_list_old[$index];
                unset($new_product['label_file']);
                unset($old_product['label_file']);
